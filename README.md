...
- 🚀 Deployable with [Workflow Server Manager (WFSM)](https://github.com/agntcy/workflow-srv-mgr)
- 🤝 **ACP protocol** compatible for multi-agent orchestration
...
This project uses the [ArgoCD MCP Server by `severity1`](https://mcp.so/server/argocd-mcp/severity1) as a **git submodule** in `argocd_mcp/`.

All ArgoCD-related LangChain tools are defined by that MCP server implementation.
...
- [ArgoCD MCP Server by severity1](https://github.com/severity1/argocd-mcp) for ArgoCD tool definitions.
